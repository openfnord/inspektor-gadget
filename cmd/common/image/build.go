...
func createEbpfDesc(target oras.Target, progFilePath string) (ocispec.Descriptor, error) {
	progFile, err := os.Open(progFilePath)
	if err != nil {
		return ocispec.Descriptor{}, fmt.Errorf("open eBPF program file: %w", err)
	}
	defer progFile.Close()
	progBytes, err := io.ReadAll(progFile)
	if err != nil {
		return ocispec.Descriptor{}, fmt.Errorf("read eBPF program file: %w", err)
	}

	progDesc := content.NewDescriptorFromBytes("application/ebpf.oci.image.config.v1+binary", progBytes)
...
func createDefinitionDesc(target oras.Target, definitionFilePath string) (ocispec.Descriptor, error) {
	definitionFile, err := os.Open(definitionFilePath)
	if err != nil {
		return ocispec.Descriptor{}, fmt.Errorf("open definition file: %w", err)
	}
	defer definitionFile.Close()
	definitionBytes, err := io.ReadAll(definitionFile)
	if err != nil {
		return ocispec.Descriptor{}, fmt.Errorf("read definition file: %w", err)
	}
	defDesc := content.NewDescriptorFromBytes("application/config.image.inspektor-gadget.io+yaml", definitionBytes)
