...
func (t *Tracer) parseParams(params *params.Params) error {
	...
	if lAddrV6 != "" {
		if lAddr != "" {
			return fmt.Errorf("using both %s and %s is not permitted", ParamFilterLocalAddress, ParamFilterLocalAddressV6)
		}
	...
	if rAddrV6 != "" {
		if rAddr != "" {
			return fmt.Errorf("using both %s and %s is not permitted", ParamFilterRemoteAddress, ParamFilterRemoteAddressV6)
		}
...
func (t *Tracer) collectResult() ([]byte, error) {
	...
	for {
		...
		if addressType == types.AddressTypeAll {
			...
			addr = gadgets.IPStringFromBytes(key.Addr, ipType)
		}
