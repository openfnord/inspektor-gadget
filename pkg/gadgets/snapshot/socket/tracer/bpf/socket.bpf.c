 ...
 */
static __always_inline void socket_bpf_seq_write(
	struct seq_file *seq,
	__u16 proto, __be32 src,
	__u16 srcp, __be32 dest,
	__u16 destp, __u8 state, __u64 ino)
{
	struct entry entry = {};

	entry.daddr = dest;
	entry.saddr = src;
	entry.dport = bpf_htons(destp);
	entry.sport = bpf_htons(srcp);
	entry.proto = proto;
	entry.state = state;
	entry.inode = ino;
