...
static int dump_tcp_sock(struct seq_file *seq, struct tcp_sock *tp)
{
	...
	if (sp->sk_family == AF_INET)
		...
		socket_bpf_seq_print_v6(seq, proto, sp->__sk_common.skc_v6_rcv_saddr.in6_u.u6_addr8,
			inet->inet_sport, sp->__sk_common.skc_v6_daddr.in6_u.u6_addr8,
			inet->inet_dport, sp->sk_state, sock_i_ino(sp));
...
static int dump_tw_sock(struct seq_file *seq, struct tcp_timewait_sock *ttw)
{
	...
	if (tw->tw_family == AF_INET)
		...
		socket_bpf_seq_print_v6(seq, proto, tw->tw_v6_rcv_saddr.in6_u.u6_addr8,
			tw->tw_sport, tw->tw_v6_daddr.in6_u.u6_addr8,
			tw->tw_dport, tw->tw_substate, 0);
...
static int dump_req_sock(struct seq_file *seq, struct tcp_request_sock *treq)
{
	...
	if (irsk->ireq_family == AF_INET)
		...
		socket_bpf_seq_print_v6(seq, proto, irsk->ir_v6_loc_addr.in6_u.u6_addr8,
			irsk->ir_num, irsk->ir_v6_rmt_addr.in6_u.u6_addr8, irsk->ir_rmt_port,
			TCP_SYN_RECV, sock_i_ino(treq->req.req.sk));
...
int ig_snap_tcp(struct bpf_iter__tcp *ctx)
{
	...
	if (sk_common->skc_family != AF_INET && sk_common->skc_family != AF_INET6)
		return 0;
