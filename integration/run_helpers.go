...
func MarshalJson(t *testing.T, m map[string]interface{}) string {
...
func RunEventToJsonObj(t *testing.T, ev *runtypes.Event) map[string]interface{} {
...
// TODO: Find a name for this. This is only a dev name
func parseMultiJSONOutputToJson(output string, normalize func(map[string]interface{})) ([]map[string]interface{}, error) {
...
// TODO: Find a name for this. This is only a dev name
func expectEntriesToMatchFoobar(t *testing.T, entries []map[string]interface{}, expectedEntries ...map[string]interface{}) error {
out:
	for _, expectedEntry := range expectedEntries { // Marshal & Unmarshal to have the right types in the map
		...
		fmt.Println("Searching for entry:", expectedEntry)
...
// ExpectEntriesToMatchJson verifies that all the entries in expectedEntries are
// matched by at least one entry in the output (Lines of independent JSON objects).
// TODO: Find a name for this. This is only a dev name
func ExpectEntriesToMatchJson(t *testing.T, output string, normalize func(map[string]interface{}), expectedEntries ...map[string]interface{}) error {
	entries, err := parseMultiJSONOutputToJson(output, normalize)
	if err != nil {
		return err
	}
	return expectEntriesToMatchFoobar(t, entries, expectedEntries...)
}
